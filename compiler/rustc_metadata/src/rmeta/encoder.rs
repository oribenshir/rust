    ...
    fn encode_info_for_closure(&mut self, hir_id: hir::HirId) {
        ...
        match ty.kind() {
            ty::Generator(..) => {
                ...
                let generator_interior_type =
                    typeck_result.generator_interior_types.map_bound_ref(|vec| {
                        vec.iter()
                            .map(|item| {
                                let def_id = item
                                    .expr
                                    .and_then(|hir_id| self.tcx.hir().opt_local_def_id(hir_id))
                                    .and_then(|local_def_id| Some(local_def_id.to_def_id()));
                                ExternalGeneratorInteriorTypeCause {
                                    ty: item.ty.clone(),
                                    span: item.span.clone(),
                                    scope_span: item.scope_span.clone(),
                                    yield_span: item.yield_span.clone(),
                                    expr: def_id,
                                }
                            })
                            .collect::<Vec<_>>()
                    });
                let generator_diagnostic_data = GeneratorDiagnosticData {
                    generator_interior_type: generator_interior_type,
                    hir_owner: typeck_result.hir_owner.to_def_id(),
